...
try {
    $kasi = new KasiLunar();       // 음력 <-> 양력 변환
    $api  = new Lunar_API();       // 간지, 세간지, 일간 등
    ...
    switch ($cmd) {
        ...
        case "dailyCalendar":
            ...
            for ($d = 1; $d <= $daysInMonth; $d++) {
                $gz = $api->getGanZhi((int)$y, (int)$m, $d, 12);
                $lunar = $kasi->tolunar(sprintf("%04d-%02d-%02d", $y, $m, $d));
                $arr[] = [
        ...
        case "monthPillar":
            [$y, $m] = array_slice($argv, 2, 2);
            $gz = $api->getGanZhi((int)$y, (int)$m, 1, 12);
